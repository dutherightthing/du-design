...
| Real-world UI styles by aesthetic; a ready DESIGN.md (colors, type, spacing, components) from a real product site | styles.refero.design (2,000+ styles, e.g. "Linear-style SaaS", "Cream paper & serif") | Refero MCP if Jerry has a plan, else Jerry picks | **Don't scrape or crawl.** Its robots.txt blocks AI crawlers by name (checked 2026-09-23). With a paid Refero plan, use the official Refero MCP (`https://api.refero.design/mcp`), which searches styles, screens and flows. Without one, ask Jerry to browse and paste a style link or its DESIGN.md, then run `design-dna` on it. Avoid unofficial "refero-styles" MCPs; they crawl the site. |
