...
| `refs(query, source?)` | Live example sites from the cached galleries (navbar.gallery, cta.gallery, landing.love, saaspo), matched on category tags. |
...
- `python3 tools/du.py refresh [source]` — re-scrape the galleries into `.cache/refs.db` (gitignored; never commit scraped content). Uses the `orth` CLI; about $0.005 per page, around $0.75 for all four. `refs` reports how old the cache is.
